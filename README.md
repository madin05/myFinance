...
MyFinance adalah platform pengelolaan keuangan personal yang dirancang untuk mencatat transaksi, menganalisis pengeluaran, mengatur anggaran, serta melacak target tabungan (wishlist). Aplikasi ini dibangun dengan Vanilla JavaScript di sisi frontend untuk performa yang optimal dan Node.js/Express di sisi backend.
...
### Keamanan & Autentikasi 2FA OTP
*   **Autentikasi Akun**: Menggunakan Firebase Authentication untuk pendaftaran dan masuk akun secara aman.
*   **Autentikasi Dua Faktor (2FA OTP)**: Sistem 2FA berbasis 6-digit OTP stateless via email dengan alur Challenge-Response (tes OTP saat aktivasi dan verifikasi password saat penonaktifan).
...
*   **ORM**: Prisma
*   **Authentication**: Firebase Admin SDK, Bcrypt & JWT Pre-Auth Tokens
...
Salin file `.env.example` menjadi `.env` di direktori utama dan folder `backend/` untuk mendefinisikan variabel lingkungan yang diperlukan (koneksi database, konfigurasi Firebase SDK, JWT secret, dll.).
