...
export function openDisable2FAModal(onSuccess, onCancel, confirmPassword = null) {
  ...
  .then(({ ok, data }) => {
      ...
      <div class="modal-overlay" id="disable-2fa-overlay">
        <div class="modal-content" style="max-width: 440px;">
          <div class="modal-header" style="display: flex; justify-content: center; align-items: center; text-align: center; border-bottom: 1px solid var(--border); padding: 1.25rem 1.5rem;">
            <h3 style="font-size: 1.15rem; font-weight: 700; color: var(--red); margin: 0; display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%;">
              <i class="ph-bold ph-shield-slash" style="font-size: 1.35rem; color: var(--red);"></i>
              Verifikasi Penonaktifan 2FA
            </h3>
          </div>
          <div class="modal-body" style="padding: 1.25rem; text-align: center;">
            ...
            <form id="form-disable-2fa-otp">
              ...
              <div style="display: flex; gap: 0.75rem; margin-top: 1.25rem;">
                <button type="button" class="btn btn-outline" style="flex: 1; height: 44px; border-radius: 10px; font-weight: 600;" id="btn-cancel-disable-2fa">Batal</button>
                <button type="submit" class="btn btn-danger" style="flex: 1; height: 44px; border-radius: 10px; font-weight: 600; background: var(--red, #ef4444); color: white; border: none;" id="btn-submit-disable-2fa">Nonaktifkan 2FA</button>
              </div>
            </form>
          </div>
        </div>
    ...
    requestAnimationFrame(() => {
      document.getElementById('disable-2fa-overlay')?.classList.add('active');
    });
